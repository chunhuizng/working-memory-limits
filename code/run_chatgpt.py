...
parser.add_argument('--model_index', type=str, default='gpt-3.5-turbo', choices=['gpt-3.5', 'gpt-4', 'gpt-3.5-turbo', 'gpt-4', 'text-ada-001', 'text-babbage-001', 'text-curie-001', 'text-davinci-001', 'text-davinci-002'])
parser.add_argument('--task', type=str, default='all', choices=['multiple_choice', 'free_form'])
parser.add_argument('--prompt', type=str, default='vanilla', choices=['vanilla', 'zero_shot_cot', 'manual_cot', 'PS', 'tot_pp', 'self_consistency', 'auto_cot', 'cotp'])
...
def run_combined(tasks, mode, prompt, model_index="gpt-3.5-turbo"):
    assert prompt in ['vanilla', 'zero_shot_cot', 'manual_cot', 'PS', 'tot_pp', 'self_consistency', 'auto_cot', 'cotp'], "Invalid run type"
    assert model_index in ['gpt-3.5-turbo', 'gpt-4', 'gpt-3.5', 'gpt-4'], "Invalid model index"
    ...
    for task in tasks:
        ...
        if prompt in ['vanilla', 'zero_shot_cot', 'manual_cot', 'self_consistency', 'PS', 'tot_pp', 'auto_cot', 'cotp']:
        ...
        with open(output_path, 'w') as fd, open(failure_path, 'w') as failure_fd:
            for q_ in tqdm(task_data['examples']):
                ...
                if prompt == 'self_consistency':
                    ans_collection = []
                    ans_model_collection = []  # Initialize list to collect ans_model values
                    for _ in range(3):  # Run the model 3 times
                        ...
                        ans_collection.append(ans_)
                        ans_model_collection.append(ans_model)  # Collect each ans_model
                    # pick the most frequent ans_ from the 3 ans_collection
                    ans_ = max(set(ans_collection), key=ans_collection.count)
                    ans_models_str = "\n".join(
                        [f"A_model_{i + 1}:\n{ans_model}" for i, ans_model in enumerate(ans_model_collection)])
